...
Процедура ОпределитьНастройкиФормы(Форма, КлючВарианта, Настройки) Экспорт
	...
	Настройки.ЗагрузитьНастройкиПриИзмененииПараметров = Отчеты.рдв_СверкаПолнотыТочностиДанных.ЗагрузитьНастройкиПриИзмененииПараметров();
КонецПроцедуры
...
Процедура ПередЗагрузкойВариантаНаСервере(Форма, Настройки, ПередЗагрузкойНастроек = Ложь) Экспорт
	...
	Если ТипЗнч(Настройки) = Тип("НастройкиКомпоновкиДанных") Или Настройки = Неопределено Тогда
		...
		Если ТипЗнч(ДвоичныеДанныеСхемы) = Тип("ДвоичныеДанные") Тогда
			...
			Схема = Отчеты.рдв_СверкаПолнотыТочностиДанных.ИзвлечьСхемуИзДвоичныхДанных(ДвоичныеДанныеСхемы);
		КонецЕсли;
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если НовыеНастройкиКД.ДополнительныеСвойства.Свойство("УстановитьФиксированныеПараметры") Тогда
		...
		Попытка
			Отчеты.рдв_СверкаПолнотыТочностиДанных.УстановитьФиксированныеПараметры(ЭтотОбъект,
		...
		Исключение
			...
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Для варианта отчета с ключом ""%1""
				           |не удалось установить фиксированные параметры по причине:
				           |%2'"),
				           КлючВарианта,
				           ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Варианты отчетов.Настройка параметров отчета'", 
	...
	ФиксированныеПараметры = Отчеты.рдв_СверкаПолнотыТочностиДанных.ФиксированныеПараметры(
	...
	Если Не ЗначениеЗаполнено(КлючСхемы) И Не ЭтоЗагруженнаяСхема Тогда
		Схема = Отчеты.рдв_СверкаПолнотыТочностиДанных.СхемаКомпоновкиДанных(ФиксированныеПараметры);
	КонецЕсли;
	...
	Если ТекущийКлючСхемы <> Неопределено И (ТекущийКлючСхемы <> КлючСхемы) Тогда
		...
		Если ЭтоЗагруженнаяСхема Тогда
			Отчеты.рдв_СверкаПолнотыТочностиДанных.УстановитьСтандартныеНастройкиЗагруженнойСхемы(
				ЭтотОбъект, ДвоичныеДанныеСхемы, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД);
		Иначе
			Отчеты.рдв_СверкаПолнотыТочностиДанных.УстановитьСтандартныеНастройки(
		...
		ФиксированныеПараметры = Отчеты.рдв_СверкаПолнотыТочностиДанных.ФиксированныеПараметры(
			НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД, ДоступныеЗначения, Контекст);
	Иначе
		Отчеты.рдв_СверкаПолнотыТочностиДанных.УстановитьФиксированныеПараметры(
			ЭтотОбъект, ФиксированныеПараметры, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД);
	КонецЕсли;
	...
	Отчеты.рдв_СверкаПолнотыТочностиДанных.УстановитьСтандартныйЗаголовокОтчета(
