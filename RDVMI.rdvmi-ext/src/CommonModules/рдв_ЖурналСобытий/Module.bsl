...
//		Если Событие = Перечисления.рдв_События.ИсходящееСообщение Тогда
//			ЖурналСобытийКонтекстПриЗаписи(Источник, Отказ);
//		КонецЕсли;
...
Процедура ЖурналСобытийКонтекстПриЗаписи(Источник, Отказ) Экспорт
...
//	
//	ДополнительныеСвойства = Источник.ДополнительныеСвойства;
//	КонтекстТранспорта = Неопределено;
//	Если ДополнительныеСвойства.Свойство("КонтекстТранспорта", КонтекстТранспорта)
//	
//	Записи = Источник.Выгрузить();
//	СобытиеКонтекста = Перечисления.рдв_События.ОбработкаСообщения;
//	Запись = Записи[0];
//	ДанныеСообщения = рдв_ИнтеграцияОбщегоНазначения.Сообщение(Запись.ИдентификаторСообщения);
//		
//	Если ДанныеСообщения.СтатусОбработки = Перечисления.рдв_СтатусыОбработкиСообщений.КОбработке Тогда
//		Продолжить;
//	КонецЕсли;
//	
//	РегистрыСведений.рдв_ЖурналСобытий.ЗарегистрироватьСобытие(Запись.ИдентификаторКонтекста,
//																	Запись.ИдентификаторСообщения,
//																	,
//																	,
//																	СобытиеКонтекста,
