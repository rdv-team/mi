...
//
Функция ОбъектИБВСообщениеОбмена(ВыгружаемыйОбъект, ФорматОбмена = Неопределено) Экспорт
	...
	ДанныеИБ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыгружаемыйОбъект, "Тип, ИдентификаторДанных, СсылкаНаОбъект, ОтборНабораЗаписей");
	...
	ТекстСообщения = СформироватьТекстСообщения(ВыгружаемыеДанные, ФорматОбмена);
...
Функция ПодготовитьДанныеИБКВыгрузке(ДанныеИБ, ПКО)
	...
	Если ДанныеИБ.Тип = Перечисления.рдв_ТипыВыгружаемыхОбъектов.Ссылка Тогда
		Возврат ПодготовитьСсылочныеДанныеКВыгрузке(ДанныеИБ, ПКО);
	ИначеЕсли ДанныеИБ.Тип = Перечисления.рдв_ТипыВыгружаемыхОбъектов.НаборЗаписей Тогда
		Возврат ПодготовитьНаборЗаписейКВыгрузке(ДанныеИБ, ПКО);
	КонецЕсли;
...
//  Текст сообщения.
//
Функция СформироватьТекстСообщения(ВыгружаемыеДанные, ФорматОбмена)

	Если ФорматОбмена = Перечисления.рдв_ФорматыОбмена.json Тогда
		Возврат рдв_ОбменДаннымиJSON.СформироватьТекстСообщения(ВыгружаемыеДанные);
	КонецЕсли;
...
Функция СообщениеОбменаВОбъектИБ(ТекстСообщения, ФорматОбмена = Неопределено) Экспорт
	...
	ЗагружаемыеДанные = ПрочитатьТекстСообщения(ТекстСообщения, ФорматОбмена);
...
Функция ПрочитатьТекстСообщения(ТекстСообщения, ФорматОбмена)
	...
	Если ФорматОбмена = Перечисления.рдв_ФорматыОбмена.json Тогда	
		Возврат рдв_ОбменДаннымиJSON.ПрочитатьТекстСообщения(ТекстСообщения);
	КонецЕсли;
