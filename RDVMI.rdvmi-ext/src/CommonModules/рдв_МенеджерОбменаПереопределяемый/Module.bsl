...
Процедура ПриВыгрузкеСсылки(Ссылка, ВыгружаемыеДанные) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("СправочникСсылка._ДемоКонтрагенты") Тогда
		ВыгружаемыеДанные.Вставить("КодПартнера", Ссылка.Партнер.Код);
	КонецЕсли;
...
Процедура ОпределитьСвойстваЗагружаемогоОбъекта(СвойстваЗагружаемогоОбъекта) Экспорт
	...
	Если СвойстваЗагружаемогоОбъекта.ЗагружаемыеДанные._ТипЗначения = "Справочник._ДемоКассы" Тогда
		Если СвойстваЗагружаемогоОбъекта.ЗагружаемыеДанные.Наименование = "Тестовая касса 333" Тогда
			СвойстваЗагружаемогоОбъекта.МенеджерОбъекта = Справочники._ДемоКассыККМ;
			СвойстваЗагружаемогоОбъекта.ПКО = рдв_МенеджерОбмена.ПравилаКонвертацииПоИмениПКО("Справочник_КассыККМ", "Получение");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьЗначениеПеречисления(ДанныеРеквизита) Экспорт
	...
	Если ДанныеРеквизита._ТипЗначения = "Перечисление.рдв_ВидКонтрагента" Тогда
		Если ДанныеРеквизита._Значение = "ФизЛицо" Тогда
			Возврат Перечисления._ДемоЮридическоеФизическоеЛицо.ФизическоеЛицо;
		Иначе 
			Возврат Перечисления._ДемоЮридическоеФизическоеЛицо.ЮридическоеЛицо;
		КонецЕсли;
	КонецЕсли;
