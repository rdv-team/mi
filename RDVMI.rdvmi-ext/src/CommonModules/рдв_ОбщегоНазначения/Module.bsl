...
Функция ТипыЗначенийСодержатСсылочныйТип(МассивТипов) Экспорт
	...
	Для Каждого Тип Из МассивТипов Цикл
		...
		Если НЕ Тип = ТипНеопределено
			И НЕ Тип = ТипNull
			И НЕ Тип = ТипЧисло
			И НЕ Тип = ТипСтрока
			И НЕ Тип = ТипБулево
			И НЕ Тип = ТипДата Тогда
...
Функция МенеджерОбъектаПоПолномуИмени(ПолноеИмя) Экспорт
	...
	Если Менеджер = Неопределено Тогда
		ВызватьИсключение(СтрШаблон(НСтр("ru = 'Неверное значение параметра %1 функции %2. У объекта метаданных ""%3"" нет менеджера объекта.'"), 
			"ПолноеИмя", "ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени", ПолноеИмя), КатегорияОшибки.ОшибкаКонфигурации);
	КонецЕсли;
