	...
	Пока ВыборкаПоВнешнейСистеме.Следующий() Цикл
		...
		Пока ВыборкаПоАдресу.Следующий() Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Статус = Неопределено;
				СообщениеТранспорта = "";
				Если ПараметрыСообщения.ТекстСообщения = "" Тогда
					Статус = Перечисления.рдв_СтатусыРегистрацииИзменений.Отменено;
				Иначе
					Результат = рдв_РаботаССервисами.ВыгрузитьСообщение(ПараметрыСообщения);
					Если Результат.ОшибкаТранспорта Тогда
						Статус = Перечисления.рдв_СтатусыРегистрацииИзменений.Ошибка;
						СообщениеТранспорта = "";
					КонецЕсли;
				КонецЕсли;
				ОбновитьДанныеРегистрации(Выборка, Статус, СообщениеТранспорта);
			КонецЦикла;
	КонецЦикла;
 	КонецЦикла;
...
Процедура ИнтеграцияRMQПолучение(ВнешняяСистема) Экспорт
	...
	Пока Клиент.ПолучитьСообщение(Результат, Таймаут, ВыполнитьПодтверждение) Цикл
		...
		ДанныеЗапроса.Вставить("ВнешняяСистема", ВнешняяСистема);
...
Процедура ОбновитьДанныеРегистрации(Выборка, Статус = Неопределено, СообщениеТранспорта = "")

	Если Статус = Перечисления.рдв_СтатусыРегистрацииИзменений.Ошибка Или
		Статус = Перечисления.рдв_СтатусыРегистрацииИзменений.Отменено Тогда
		РегистрыСведений.рдв_РегистрацияИзменений.ОбновитьСтатусРегистрации(Выборка.ИдентификаторСообщения, Статус, СообщениеТранспорта);
	Иначе
