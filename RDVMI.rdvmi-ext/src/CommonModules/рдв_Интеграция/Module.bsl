...
Процедура ОтправкаСообщенийRMQ(ВнешняяСистема, ИдентификаторСообщения = Неопределено) Экспорт
	...
	Пока ВыборкаПоВнешнейСистеме.Следующий() Цикл
		ВыборкаПоВидуСообщения = ВыборкаПоВнешнейСистеме.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаПоВидуСообщения.Следующий() Цикл
			...
			ВыборкаПоАдресу = ВыборкаПоВидуСообщения.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаПоАдресу.Следующий() Цикл
			
				ЗаполнитьЗначенияСвойств(ПараметрыПодключения, ВыборкаПоАдресу);
				...
				Пока Выборка.Следующий() Цикл
					...
					ДанныеЗапроса = рдв_РаботаСRMQ.ЗаполнитьДанныеЗапросаОтправка(Выборка, ПараметрыПодключения);
					ДанныеЗапроса.Вставить("Тело", ТекстСообщения);
					
					Результат = Клиент.ОтправитьСообщение(ТекстСообщения, Выборка.ИмяВидаЗапроса, Выборка.ИдентификаторСообщения, Выборка.ИдентификаторСобытия);
					
					ДанныеОтвета = рдв_РаботаСRMQ.ЗаполнитьДанныеОтветаОтправка(Результат);
					
					МассивОбъектов = Новый Массив;
					Если ЗначениеЗаполнено(Выборка.СсылкаНаОбъект) Тогда
						МассивОбъектов.Добавить(Выборка.СсылкаНаОбъект);
					КонецЕсли;
					ДанныеСервиса = рдв_РаботаСRMQ.ЗаполнитьДанныеСервисаОтправка(Выборка, МассивОбъектов);
					...
					Если Не Результат.Успешно Тогда
						Ошибка = Перечисления.рдв_СтатусыРегистрацииИзменений.Ошибка;
						РегистрыСведений.рдв_РегистрацияИзменений.ОбновитьСтатусРегистрации(Выборка.ИдентификаторСообщения, Ошибка, Результат.ТекстОшибки);
					Иначе
						РегистрыСведений.рдв_РегистрацияИзменений.УдалитьРегистрацию(Выборка.ИдентификаторСообщения, Выборка.ВыгружаемыйОбъект);
					КонецЕсли;
