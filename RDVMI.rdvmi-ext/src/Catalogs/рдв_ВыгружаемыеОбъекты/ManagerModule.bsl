...
Функция СоздатьВыгружаемыйОбъектСсылка(Источник, ИдентификаторДанных)
	...
	НовыйОбъект.ИдентификаторДанных = ИдентификаторДанных;
	НовыйОбъект.СсылкаНаОбъект = СсылкаНаОбъект;
	НовыйОбъект.Записать();
	Возврат НовыйОбъект.Ссылка;
	
КонецФункции

Функция СоздатьВыгружаемыйОбъектПодтверждениеПолучения(ИдентификаторДанных, ИдентификаторСобытия, Успешно, ТекстОшибки) Экспорт

	НовыйОбъект = Справочники.рдв_ВыгружаемыеОбъекты.СоздатьЭлемент();
	НовыйОбъект.Тип = Перечисления.рдв_ТипыВыгружаемыхОбъектов.ПодтверждениеПолучения;
	НовыйОбъект.Наименование = СтрШаблон("Подтверждение получения %1", ИдентификаторДанных);
	НовыйОбъект.ИдентификаторДанных = ИдентификаторДанных;
	НовыйОбъект.ИдентификаторСобытия = ИдентификаторСобытия;
	НовыйОбъект.Записать();
	Возврат НовыйОбъект.Ссылка;
