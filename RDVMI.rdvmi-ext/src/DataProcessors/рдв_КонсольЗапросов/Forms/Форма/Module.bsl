...
Процедура ОбновитьФормуКлиент()
	...
	Для каждого СтрЗапросы Из Объект.Запросы Цикл
		...
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда
			...
			ТекстЗапроса.УстановитьТекст(СтрЗапросы.Текст);
...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	Иначе
		ТекущийЗапрос.АдресРезультатовЗапроса = ПоместитьВоВременноеХранилище(Результат, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ЧтениеФайлаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		// АПК:1348-выкл - обработка может использоваться вне БСП, поэтому вызов процедуры общего модуля
		//                 "ФайловаяСистемаКлиент" не требуется.
		НачатьПомещениеФайлов(Оповещение,, Диалог, Истина, УникальныйИдентификатор);
		// АПК:1348-вкл
	Иначе
		СообщитьПользователю(НСтр("ru = 'Без расширения для работы с 1С:Предприятием невозможно сохранять и загружать запросы в файл.'"), "Объект");
	КонецЕсли;
...
Процедура ЗаполнитьПараметрыИзXMLСервер(Знач СтрокаXML, Знач Контекст)
	...
	Исключение
		...
		СообщитьПользователю(ПодставитьПараметрыВСтроку(ТекстОшибки, ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат;
	КонецПопытки;
