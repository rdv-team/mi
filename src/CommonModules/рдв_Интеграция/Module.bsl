...
// Возвращаемое значение:
//  Булево 
Функция ИнтеграцияВнешняяСистемаДоступна(ВнешняяСистема) Экспорт
	...
	ДополнительныеПараметры = Неопределено;
	Попытка
		Доступна = МодульИнтеграции.ИнтеграцияДоступна(ПараметрыСоединения, ДополнительныеПараметры);
	Исключение
		Возврат Ложь;
	КонецПопытки;
...
Процедура СообщениеОбработатьПолученное(ИдентификаторСообщения) Экспорт
	...
	Для Каждого Объект Из Объекты Цикл
		...
		Если ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(Объект.Ключ.Метаданные()) Тогда
			МассивЗагруженныхОбъектов.Добавить(Объект.Ключ.Ссылка);
		КонецЕсли;
	...
	Попытка
		...
		ДатаЗаписиНачалоЧаса = Сообщение.ДатаЗаписиНачалоЧаса;
		КонтекстТранспорта = Новый_КонтекстТранспорта();
		ЗаполнитьЗначенияСвойств(КонтекстТранспорта, Сообщение);
		КонтекстТранспорта.ДатаСобытия		= ТекущаяДатаСеанса();
		КонтекстТранспорта.ДатаСобытияМс	= ТекущаяУниверсальнаяДатаВМиллисекундах();
		ЗаписатьОбъектыСообщенияИнтеграции(КонтекстТранспорта ,МассивЗагруженныхОбъектов , ДатаЗаписиНачалоЧаса);
		ЗафиксироватьТранзакцию();
	...
	Исключение
		...
		Событие = НСтр("ru ='Обработка входящего сообщения. Обновление состояния сообщения и объектов интеграции.'");
		Контекст = Новый_ЗаписатьОшибкуИнтеграции(Сообщение.ВнешняяСистема, Событие, Сообщение.ИдентификаторСообщения);
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ЗаписатьОшибкуИнтеграции(ИнформацияОбОшибке, Контекст);
...
// Параметры:
//  Параметры Параметры
Процедура РегламентОчисткиСлужебныхДанных(Параметры=Неопределено) Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.рдв_ОчисткаСлужебныхДанных);
	
	КоличествоДней = Константы.рдв_КоличествоДнейХраненияДанных.Получить();
	Если КоличествоДней = 0 Тогда
		КоличествоДней = 60;
	КонецЕсли;
	ДатаОтсечения = НачалоДня(ТекущаяДатаСеанса())-(86400*КоличествоДней);
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДатаОтсечения", ДатаОтсечения);
	
	ИменаРегистров = Новый Массив;
	ИменаРегистров.Добавить("рдв_СообщенияИнтеграции");
	ИменаРегистров.Добавить("рдв_КонтекстСообщенийИнтеграции");
	ИменаРегистров.Добавить("рдв_ОбъектыСообщенийИнтеграции");
	ИменаРегистров.Добавить("рдв_ЖурналСобытий");
	ИменаРегистров.Добавить("рдв_ОшибкиИнтеграции");
	
	Для Каждого Регистр ИЗ ИменаРегистров Цикл
		
		Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РегистрКОчистке.ДатаЗаписиНачалоЧаса КАК ДатаЗаписиНачалоЧаса
		|ИЗ
		|	&ИмяРегистра КАК РегистрКОчистке
		|ГДЕ
		|	РегистрКОчистке.ДатаЗаписиНачалоЧаса <= &ДатаОтсечения
		|
		|УПОРЯДОЧИТЬ ПО
		|	ДатаЗаписиНачалоЧаса";
		Запрос.Текст = СтрЗаменить(Текст, "&ИмяРегистра", "РегистрСведений."+Регистр);
		//@skip-check query-in-loop
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Набор.ДополнительныеСвойства.Вставить("рдв_СлужебноеУдаление", Истина);
			Набор.Записать();
