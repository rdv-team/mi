...
Функция МодульПрикладнойЛогики() Экспорт
	...
	Возврат рдв_ИнтеграцияПереопределяемый.МодульПрикладнойЛогикиРегистрацииИзменений();
...
//	Замещение - Вариант замещения для набора записей
//
Процедура ЗарегистрироватьИзменения(Источник, ИдентификаторСобытия = Неопределено, ВнешниеСистемы = Неопределено
	...
	Для Каждого ПравилоРегистрации Из СтрокиПравил Цикл
		...
		ВыгружаемыйОбъект = ВыгружаемыйОбъект(Источник, Замещение);
...
Функция ВыгружаемыйОбъект(Источник, Замещение)
	
	ВыгружаемыйОбъект = Справочники.рдв_ВыгружаемыеОбъекты.СоздатьОбновитьВыгружаемыйОбъект(Источник,,,, Замещение);
...
Функция ВидСообщения(ВнешняяСистема, ВыгружаемыйОбъект)
	...
	Если Не ЗначениеЗаполнено(МаршрутСообщения) Тогда
		МаршрутСообщения = ИсточникМаршрута.Метаданные().Имя;
	КонецЕсли;
