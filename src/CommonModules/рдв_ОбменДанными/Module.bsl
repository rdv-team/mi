...
Функция ОбъектИБКДаннымВыгрузки(ВыгружаемыйОбъект) Экспорт
	...
	ДанныеИБ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыгружаемыйОбъект, "Тип, ИдентификаторДанных, СсылкаНаОбъект, Замещение, 
																		|	ОтборНабораЗаписей, НаборЗаписей, Успешно, ТекстОшибки");
	ДанныеИБ.ОтборНабораЗаписей = ДанныеИБ.ОтборНабораЗаписей.Выгрузить();
	ДанныеИБ.НаборЗаписей 		= ДанныеИБ.НаборЗаписей.Выгрузить();
...
Функция СообщениеОбменаВОбъектИБ(Сообщение) Экспорт
	...
	Для Каждого ОбъектСообщения Из ОбъектыСообщения Цикл
		...
		Замещение = Истина;
		Если ЗагружаемыеДанные.Свойство("Замещение") Тогда
			Если ЗагружаемыеДанные.Замещение = "Слияние" Тогда
				Замещение = РежимЗамещения.Слияние;
			ИначеЕсли ЗагружаемыеДанные.Замещение = "Удаление" Тогда
				Замещение = РежимЗамещения.Удаление;
			КонецЕсли;
		КонецЕсли;
		
		Попытка
			ЗаписатьОбъект(ЗагружаемыйОбъект, Замещение);
			ЗаписатьОбъектСБизнесЛогикой(ЗагружаемыйОбъект, Замещение);
...
Функция ПодготовитьНаборЗаписейКВыгрузке(ДанныеИБ, ПКО)
	...
	Если ДанныеИБ.Замещение = "Слияние" Или 
		ДанныеИБ.Замещение = "Удаление" Тогда
		
		ТаблицаНабора = ПодготовитьТаблицуНабораЗаписей(ДанныеИБ, ПКО);
		ТекстЗапроса = ТекстЗапросаКОтборуНабораЗаписей(ТаблицаНабора, ПКО);
		РезультатЗапроса = ВыполнитьЗапрос(ТаблицаНабора, ТекстЗапроса);
		ВыгружаемыеДанные = ОбработатьРезультатЗапросаКВыгрузке(РезультатЗапроса, ДанныеИб, ПКО);
		//ВыгружаемыеДанные.Вставить("НаборЗаписей", НаборЗаписей);
		ВыгружаемыеДанные.Вставить("Замещение", ДанныеИБ.Замещение);
		ВыгружаемыеДанные.Вставить("Отбор",  Новый Структура());
	Иначе
...
Функция ПодготовитьТаблицуОтбора(ДанныеИБ, ПКО)
	...
	Если ДанныеИБ.ОтборНабораЗаписей.Количество() > 0 Тогда
...
Функция ПодготовитьТаблицуНабораЗаписей(ДанныеИБ, ПКО)
	
	ТипыКолонок = Новый Соответствие;
	ТаблицаНабора = Новый ТаблицаЗначений;
	Для Каждого СтрокаНабора Из ДанныеИБ.НаборЗаписей Цикл
		Если СтрокаНабора.Значение = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		ЗначениеСоответствия = ТипыКолонок.Получить(СтрокаНабора.Имя);
		Если ЗначениеСоответствия = Неопределено Тогда
			МассивТипов = Новый Массив;
			МассивТипов.Добавить(ТипЗнч(СтрокаНабора.Значение));
			ТипыКолонок.Вставить(СтрокаНабора.Имя, МассивТипов);
		Иначе
			МассивТипов = ЗначениеСоответствия;
			МассивТипов.Добавить(ТипЗнч(СтрокаНабора.Значение));
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого КлючИЗначение Из ТипыКолонок Цикл
		ТаблицаНабора.Колонки.Добавить(КлючИЗначение.Ключ, Новый ОписаниеТипов(КлючИЗначение.Значение));
	КонецЦикла;
	
	НомерЗаписи = 0;
	Для Каждого СтрокаНабора Из ДанныеИБ.НаборЗаписей Цикл
		Если НомерЗаписи <> СтрокаНабора.НомерЗаписи Тогда
			НомерЗаписи = СтрокаНабора.НомерЗаписи;
			НоваяСтрока = ТаблицаНабора.Добавить();
		КонецЕсли;
		НоваяСтрока[СтрокаНабора.Имя] = СтрокаНабора.Значение;
	КонецЦикла;
	
	Возврат ТаблицаНабора;
...
Функция ВыполнитьЗапрос(ДанныеИБ, ТекстЗапроса)
	...
	Если ТипЗнч(ДанныеИБ) = Тип("ТаблицаЗначений") Тогда
		Запрос.УстановитьПараметр("ТаблицаЗначений", ДанныеИБ);
	ИНачеЕсли ДанныеИБ.Тип = Перечисления.рдв_ТипыВыгружаемыхОбъектов.Ссылка Тогда
...
Функция ТекстЗапросаКОтборуНабораЗаписей(ТаблицаОтбора, ПКО)
	...
	|ИЗ
	|	&ТаблицаЗначений КАК ОбъектВыгрузки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ПолучитьСвойстваЗагружаемогоОбъекта(ЗагружаемыеДанные)
	...
	Если СвойстваЗагружаемогоОбъекта.МенеджерОбъекта = Неопределено Тогда
		...
		Если Лев(ТипЗначения, 8) = "Документ" Тогда
			СвойстваЗагружаемогоОбъекта.ЭтоДокумент = Истина;
		ИначеЕсли Лев(ТипЗначения, 7) = "Регистр" Тогда
...
//	ЗагружаемыйОбъект - СправочникОбъект, ДокументОбъект, НаборЗаписей - Загружаемый объект
//
Процедура ЗаписатьОбъектСБизнесЛогикой(ЗагружаемыйОбъект, Замещение = Истина)
	...
	ИначеЕсли ЭтоРегистр Тогда
		ЗагружаемыйОбъект.Записать(Замещение);
	Иначе 
		Если ЗначениеЗаполнено(ЗагружаемыйОбъект.Ссылка) Тогда
...
Процедура ЗаписатьОбъект(ЗагружаемыйОбъект, Замещение = Истина)
