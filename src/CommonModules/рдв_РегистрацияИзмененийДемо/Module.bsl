...
Процедура ЗаполнитьПравилаРегистрации(ПравилаРегистрации) Экспорт
	...
	ДобавитьПравилоРегистрации(ПравилаРегистрации, 
								Метаданные.Справочники._ДемоОрганизации, 
								RMQ,
								"exchange");
...
Функция МаршрутСообщения(ИсточникСообщения, ВнешняяСистема) Экспорт
	...
	МетаданныеИсточника = ИсточникСообщения.Метаданные();
	Если МетаданныеИсточника = Метаданные.Справочники._ДемоКонтрагенты Тогда
		Возврат "Контрагенты";
	ИначеЕсли МетаданныеИсточника = Метаданные.Справочники._ДемоПартнеры Тогда
		Возврат "Партнеры";
	ИначеЕсли МетаданныеИсточника = Метаданные.Справочники._ДемоКассы Тогда
		Возврат "Кассы";
	ИначеЕсли МетаданныеИсточника = Метаданные.Документы._ДемоЗаказПокупателя Тогда
		Возврат "ЗаказПокупателя";
	ИначеЕсли МетаданныеИсточника = Метаданные.Справочники.рдв_ВыгружаемыеОбъекты Тогда
	...
	Иначе
		Возврат МетаданныеИсточника.Имя;
	КонецЕсли;
