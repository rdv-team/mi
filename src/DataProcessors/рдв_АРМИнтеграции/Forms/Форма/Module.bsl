...
Функция ДобавитьЭлементНаФорму(Запись, Родитель)
	...
	Если ТипЗнч(Запись.ВидСообщения) = Тип("СправочникСсылка.рдв_ВидыСообщенийИнтеграции") Тогда
		ИмяВидаСообщения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.ВидСообщения, "ИмяВидаСообщения");
	Иначе
		ИмяВидаСообщения = Запись.ВидСообщения;
	КонецЕсли;
	ИмяЭлемента = СтрШаблон("%1_%2", Родитель.Имя, СтрЗаменить(ИмяВидаСообщения, ".", ""));
