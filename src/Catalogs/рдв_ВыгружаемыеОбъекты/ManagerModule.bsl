...
Функция СоздатьОбновитьВыгружаемыйОбъект(Источник = Неопределено, ИдентификаторДанных = Неопределено
										, ВыгружаемыйОбъект = Неопределено, Входящий = Ложь
										, Замещение = Неопределено) Экспорт
	
	Возврат СоздатьОбновить(Источник, ИдентификаторДанных, ВыгружаемыйОбъект, Входящий, Замещение);
...
Функция СоздатьОбновить(Источник, ИдентификаторДанных, ВыгружаемыйОбъект = Неопределено, Входящий = Ложь, Замещение = Неопределено)
	
	Описание = ОписаниеИсточника(Источник, ИдентификаторДанных, ,,,Замещение);
...
Функция ОписаниеОбъекта() Экспорт
	...
	Результат.Вставить("Замещение", "");
	ТЗ = Новый ТаблицаЗначений();
	ТЗ.Колонки.Добавить("Имя", ОбщегоНазначения.ОписаниеТипаСтрока(100));
	ТЗ.Колонки.Добавить("Значение");
	Результат.Вставить("ОтборНабораЗаписей", ТЗ);
	ТЗ = Новый ТаблицаЗначений();
	ТЗ.Колонки.Добавить("НомерЗаписи", ОбщегоНазначения.ОписаниеТипаЧисло(0));
	ТЗ.Колонки.Добавить("Имя", ОбщегоНазначения.ОписаниеТипаСтрока(100));
	ТЗ.Колонки.Добавить("Значение");
	Результат.Вставить("НаборЗаписей", ТЗ);
...
Функция ОписаниеИсточника(Источник = Неопределено, ИдентификаторДанных = Неопределено,
									...
									ВидСообщения = Неопределено, Замещение = Неопределено)
	...
	Иначе
		...
		Если Лев(ИмяМетаданных, 7) = "Регистр" Тогда
			...
			Если НЕ МетаданныеИсточника = Метаданные.РегистрыСведений.рдв_СообщенияИнтеграции Тогда
				...
				КонецЦикла;
				Если ТипЗнч(Замещение) = Тип("Булево") Или
					Замещение = Неопределено Тогда
				Иначе
					Описание.Замещение = Замещение;
					ТаблицаРегистра = Источник.ВыгрузитьКолонки();
					НомерСтроки = 0;
					Для Каждого Запись Из Источник Цикл
						НомерСтроки = НомерСтроки + 1;
						Для Каждого Колонка Из ТаблицаРегистра.Колонки Цикл
							НоваяСтрока = Описание.НаборЗаписей.Добавить();
							НоваяСтрока.НомерЗаписи = НомерСтроки;
							НоваяСтрока.Имя = Колонка.Имя;
							НоваяСтрока.Значение = Запись[Колонка.Имя];
						КонецЦикла;
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
